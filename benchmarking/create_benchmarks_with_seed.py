...
def main():
    ...
    for VOC_freq in VOC_frequencies:
        ...
        print("Simulating reads from {} at {}x coverage".format(fasta_selection,
                                                                background_cov))
        subprocess.check_call("art_illumina --rndSeed {3} -ss HS25 -i {0} -l 150 -f {1} -p -o {2}/background_{1}x -m 250 -s 10".format(fasta_selection, background_cov, args.outdir, args.seed), shell=True)
        ...
        for filename in VOC_files:
            ...
            print("Simulating reads from {} at {}x coverage".format(VOC_name,
                                                                    VOC_cov))
            subprocess.check_call("art_illumina --rndSeed {4} -ss HS25 -i {0} -l 150 -f {1} -p -o {2}/{3}_{1}x -m 250 -s 10".format(voc_fasta, VOC_cov, args.outdir, VOC_name, args.seed), shell=True)
            print("\nMerging fastqs...")
